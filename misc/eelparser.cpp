...
QString EELParser::findVariable(QString key, EELPropertyType type){
    ...
    LogHelper::warning(QString("EELParser: Unable to find a supported variable definition of '%1' in script '%2'").arg(key).arg(getDescription()));
    return NORESULT;
}

bool EELParser::replaceVariable(QString key, QString value, EELPropertyType type){
    if(type == EELPropertyType::NumberRange){
        QRegularExpression re(QString(R"(%1\s*=\s*(?<val>-?\d+\.?\d*)\s*;)").arg(key));
