...
public class HeadsetService extends Service
{
	...
	*/
	...
	private float[] DRC_Input_Levels;
	private float[] DRC_Output_Levels;

	private float[] drcLevels = new float[30];
	/**
	...
	* @param levels
	*/
	public void setDRCLevels(float[] in, float[] out)
	{
		DRC_Input_Levels = in;
	...
	*/
	...
	private void updateDsp(SharedPreferences preferences, JDSPModule session, boolean updateMajor, int sessionId)
	{
		...
		if (masterSwitch)
		{
			...
			{
				float preEmp = Float.valueOf(preferences.getString("dsp.compression.pregain", "15"));
				float thres1 = Float.valueOf(preferences.getString("dsp.compression.thres1", "-35"));
				float thres2 = Float.valueOf(preferences.getString("dsp.compression.thres2", "0"));
				float maxAttack = Float.valueOf(preferences.getString("dsp.compression.maxatk", "60"));
				float maxRelease = Float.valueOf(preferences.getString("dsp.compression.maxrel", "800"));
				float compConfig[] = new float[]{ preEmp, thres1, thres2, maxAttack, maxRelease };
			...
			{
				float spm = Float.valueOf(preferences.getString("dsp.bass.spectralsmooth", "5"));
				float gm = Float.valueOf(preferences.getString("dsp.bass.gainsmooth", "10"));
				float basscond = Float.valueOf(preferences.getString("dsp.boostcond", "0.5"));
				float maxg = Float.valueOf(preferences.getString("dsp.bass.maxgain", "0"));
				float bassConfig[] = new float[]{ spm, gm, basscond, maxg };
				session.setParameterFloatArray(session.JamesDSP, 112, bassConfig);
			}
			...
			if (equalizerEnabled == 1)
			{
				...
				if (DRC_Input_Levels != null && DRC_Output_Levels != null)
				{
					...
					{
						drcLevels[i] = DRC_Input_Levels[i];
						drcLevels[i + 15] = DRC_Output_Levels[i];
					}
				}
				else
				{
					...
					for (short i = 0; i < levels.length; i++)
						drcLevels[i] = Float.valueOf(levels[i]);
				}
				...
				float sendAry[] = mergeFloatArray(ftype, drcLevels);
			...
			if (convolverEnabled == 1 && updateMajor)
			{
				...
				if(oldImpulseName.equals(mConvIRFilePath) && numberOfParameterCommitted != 0)
					updateNow = false;
				if (updateNow)
				{
