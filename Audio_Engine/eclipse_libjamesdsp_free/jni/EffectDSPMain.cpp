...
EffectDSPMain::EffectDSPMain()
	: DSPbufferLength(1024), inOutRWPosition(0), equalizerEnabled(0), ramp(1.0f), pregain(12.0f), threshold(-60.0f), knee(30.0f), ratio(12.0f), attack(0.001f), release(0.24f), isBenchData(0), mPreset(0), reverbEnabled(0), threadResult(0)
...
//						LOGI("bench_c1: %lf", benchmarkValue[1][i]);
		...
		if (cep->psize == 4 && cep->vsize == 16)
		{
			...
			if (cmd == 9999)
			{
				...
				convGaindB = (float)((int32_t *)cep)[6] / 262144.0f;
				if (convGaindB > 50.0f)
					convGaindB = 50.0f;
...
int32_t EffectDSPMain::process(audio_buffer_t *in, audio_buffer_t *out)
{
	if (inputBuffer[0])
	{
		...
		for (framePos = 0; framePos < actualFrameCount; framePos++)
		{
			...
			{
			case 0:
				inputBuffer[0][pos] = (float)in->s16[framePos2x] * 3.051757812500000e-05f;
				out->s16[framePos2x] = (int16_t)(outputBuffer[0][pos] * 32768.0f);
				inputBuffer[1][pos] = (float)in->s16[++framePos2x] * 3.051757812500000e-05f;
				out->s16[framePos2x] = (int16_t)(outputBuffer[1][pos] * 32768.0f);
			break;
			case 1:
				inputBuffer[0][pos] = in->f32[framePos2x];
				out->f32[framePos2x] = outputBuffer[0][pos];
				inputBuffer[1][pos] = in->f32[++framePos2x];
				out->f32[framePos2x] = outputBuffer[1][pos];
			break;
			case 2:
				inputBuffer[0][pos] = (float)((double)in->s32[framePos2x] * 4.656612875245797e-10);
				out->s32[framePos2x] = (int32_t)((double)outputBuffer[0][pos] * 2147483647.0);
				inputBuffer[1][pos] = (float)((double)in->s32[++framePos2x] * 4.656612875245797e-10);
				out->s32[framePos2x] = (int32_t)((double)outputBuffer[1][pos] * 2147483647.0);
			break;
			}
			pos++;
