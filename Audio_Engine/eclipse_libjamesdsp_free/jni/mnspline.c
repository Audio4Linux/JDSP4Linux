...
inline size_t b_search(const float *pxa, float x, size_t idxlow, size_t idxhigh)
{
	size_t ilo = idxlow;
	size_t ihi = idxhigh;
	size_t mid;
...
inline int lin_search(const float *pxa, float x, size_t idxlow, size_t idxhigh, size_t *index)
{
	int found = 0;
...
{
	float *pu;
	if ((pu = (float*)malloc((n - 1) * sizeof(float))) == NULL)
		return 0;
	return pu;
}
...
{
    float qn = 0.0f; /* Upper boundary condition set to be 'natural' */
    float un = 0.0f;  /* */
    float p;
    float sig;
    py2[0] = 0.0f; /* Lower boundary condition set to be 'natural' */
    pu[0] = 0.0f;  /* */
    ...
    for (size_t i = 1; i < n - 1; i++)
    {
        ...
        pu[i]   = (6.0f * pu[i] / (px[i+1] - px[i-1]) - sig * pu[i-1]) / p; 
    }
    py2[n-1] = (un - qn * pu[n-2]) / (qn * py2[n-2] + 1.0f);
...
//    #pragma omp parallel for firstprivate(prev_idx) private(klo, khi, h, b, a, x)
    for (size_t i = 0; i < nx; i++)
    {
       ...
       /* COMPUTE */
        ...
        a     = (pxa[khi] - px[i]) / h;
        b     = (px[i] - pxa[klo]) / h;
        py[i] = a * pya[klo] + b * pya[khi] + ((a*a*a -a) * py2a[klo] + (b*b*b - b) * py2a[khi]) * h*h / 6.0f;
    }
    return 0;
}