...
void on_message_error([[maybe_unused]] const GstBus* gst_bus, GstMessage* message, PipelineManager* p) {
    GError* err;
    gchar* debug;
...
void on_stream_status([[maybe_unused]] GstBus* bus, GstMessage* message, PipelineManager* p) {
...
void on_buffer_changed([[maybe_unused]] GObject* gobject, [[maybe_unused]] GParamSpec* pspec, PipelineManager* p) {
    GstState state;
    GstState pending;
...
void on_message_state_changed([[maybe_unused]] const GstBus* gst_bus, GstMessage* message, PipelineManager* p) {
...
void on_message_latency([[maybe_unused]] const GstBus* gst_bus, GstMessage* message, PipelineManager* p) {
...
void on_latency_changed([[maybe_unused]] GObject* gobject, [[maybe_unused]] GParamSpec* pspec, PipelineManager* p) {
    GstState state;
    GstState pending;
...
auto on_sink_event([[maybe_unused]] GstPad* pad, GstPadProbeInfo* info, gpointer user_data) -> GstPadProbeReturn {
    ...
    if (event->type == GST_EVENT_LATENCY) {
        auto p = static_cast<PipelineManager*>(user_data);
        p->getLatency();
    }
...
PipelineManager::PipelineManager()
{
    ...
    pm->sink_input_added.connect(sigc::mem_fun(*this, &PipelineManager::onAppAdded));
    pm->sink_input_changed.connect(sigc::mem_fun(*this, &PipelineManager::onAppChanged));
    pm->sink_input_removed.connect(sigc::mem_fun(*this, &PipelineManager::onAppRemoved));
    pm->sink_changed.connect(sigc::mem_fun(*this, &PipelineManager::onSinkChanged));
...
PulseManager* PipelineManager::getPulseManager()
{
    return pm;
}

void PipelineManager::setSource(SourceElement* _source){
    source = _source;
...
void PipelineManager::setSink(SinkElement *value)
{
    sink = value;
...
void PipelineManager::setStreamProperties(const std::string& props)
{
...
void PipelineManager::setNullPipeline() {
...
void PipelineManager::updatePipelineState() {
...
void PipelineManager::pauseAndStart(){
    gst_element_set_state(pipeline, GST_STATE_PAUSED);
    gst_element_set_state(pipeline, GST_STATE_PLAYING);
}

void PipelineManager::setState(GstState state){
    gst_element_set_state(pipeline, state);
}
/**
 * @note Only applies if the source element supports device selection
 */
void PipelineManager::setSourceMonitorName(const std::string& name) {
 ...
 */
void PipelineManager::setOutputSinkName(const std::string& name) {
    g_object_set(sink->getGstElement(), "device", name.c_str(), nullptr);

    util::debug(LOGTAG + "using output device: " + name);
}

void PipelineManager::setPulseaudioProps(const std::string& props) {
    auto str = "props," + props;
...
void PipelineManager::getLatency() {
...
void PipelineManager::setCaps(const uint& sampling_rate) {
...
void PipelineManager::onAppAdded(const std::shared_ptr<AppInfo>& app_info) {
...
void PipelineManager::onAppChanged(const std::shared_ptr<AppInfo>& app_info) {
    std::replace_copy_if(
...
void PipelineManager::onAppRemoved(uint idx) {
...
void PipelineManager::onSinkChanged(const std::shared_ptr<mySinkInfo>& sink_info) {
...
SourceElement *PipelineManager::getSource() const
{
    return source;
}

SinkElement *PipelineManager::getSink() const
{
    return sink;
}
...
void PipelineManager::runLoop(){
    g_main_loop_run (loop);
}

void PipelineManager::terminateLoop()
{
    g_main_loop_quit(loop);
}

GstElement *PipelineManager::getPipeline() const
{
    return pipeline;
}
