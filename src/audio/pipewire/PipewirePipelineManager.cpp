...
struct node_data {
  ...
  PipewirePipelineManager* pm = nullptr;
...
auto on_metadata_property(void* data, uint32_t id, const char* key, const char* type, const char* value) -> int {
  auto* pm = static_cast<PipewirePipelineManager*>(data);
  ...
  if (str_key == "default.audio.sink") {
    ...
    PipewirePipelineManager::json_object_find(str_value.c_str(), "name", v.data(), v.size() * sizeof(char));
    ...
    for (const auto& [id, node] : pm->node_map) {
      if (node.name == v.data()) {
        ...
        if (node.media_class == "Audio/Sink") {
          ...
          Glib::signal_idle().connect_once([pm, _node] { pm->new_default_sink.emit(_node); });
        }
    ...
    for (const auto& [id, node] : pm->node_map) {
      if (node.name == v.data()) {
        ...
        if (node.media_class == "Audio/Source" || node.media_class == "Audio/Source/Virtual") {
          ...
          const auto& _node = node;
          Glib::signal_idle().connect_once([pm, _node] { pm->new_default_source.emit(_node); });
        }
...
void on_registry_global(void* data,
  ...
  if (g_strcmp0(type, PW_TYPE_INTERFACE_Node) == 0) {
    if (const auto* key_media_role = spa_dict_lookup(props, PW_KEY_MEDIA_ROLE)) {
      if (util::contains(pm->blocklist_media_role, std::string(key_media_role))) {
        return;
      }
    ...
    if (const auto* key_media_class = spa_dict_lookup(props, PW_KEY_MEDIA_CLASS)) {
      ...
      if (media_class == "Audio/Sink" || media_class == "Audio/Source" || media_class == "Audio/Source/Virtual" ||
        ...
        if (name.empty()) {
          return;
        } else if (util::contains(pm->blocklist_node_name, name)) {
          return;
        }
...
PipewirePipelineManager::PipewirePipelineManager() {
...
auto PipewirePipelineManager::stream_is_connected(const uint& id, const std::string& media_class) -> bool {
...
void PipewirePipelineManager::connect_stream_output(const uint& id, const std::string& media_class) const {
  if (media_class == "Stream/Output/Audio") {
    lock();
...
void PipewirePipelineManager::disconnect_stream_output(const uint& id, const std::string& media_class) const {
  if (media_class == "Stream/Output/Audio") {
    lock();
...
void PipewirePipelineManager::connect_stream_input(const uint& id, const std::string& media_class) const {
...
void PipewirePipelineManager::disconnect_stream_input(const uint& id, const std::string& media_class) const {
  if (media_class == "Stream/Input/Audio") {
    lock();
...
void PipewirePipelineManager::set_node_volume(pw_proxy* proxy, const int& n_vol_ch, const float& value) {
  std::array<float, SPA_AUDIO_MAX_CHANNELS> volumes{};

  for(uint i = 0; i < SPA_AUDIO_MAX_CHANNELS; i++)
  {
      volumes[i] = 0.0F;
  }
...
void PipewirePipelineManager::set_node_mute(pw_proxy* proxy, const bool& state) {
  std::array<char, 1024> buffer{};
...
auto PipewirePipelineManager::link_nodes(const uint& output_node_id,
...
void PipewirePipelineManager::lock() const {
  pw_thread_loop_lock(thread_loop);
}

void PipewirePipelineManager::unlock() const {
  pw_thread_loop_unlock(thread_loop);
}

void PipewirePipelineManager::sync_wait_unlock() const {
...
void PipewirePipelineManager::destroy_object(const int& id) const {
  lock();
...
void PipewirePipelineManager::destroy_links(const std::vector<pw_proxy*>& list) const {
  for (auto* proxy : list) {
    if (proxy != nullptr) {
      lock();
...
auto PipewirePipelineManager::json_object_find(const char* obj, const char* key, char* value, const size_t& len) -> int {
