...
void on_node_info(void* object, const struct pw_node_info* info) {
  ...
  if (g_strcmp0(spa_dict_lookup(info->props, PW_KEY_STREAM_CAPTURE_SINK), "true") == 0 &&
      PwPipelineManager::exclude_monitor_stream) {
    remove_node = true;
  }
...
auto on_metadata_property(void* data, uint32_t id, const char* key, const char* type, const char* value) -> int {
  auto* const pm = static_cast<PwPipelineManager*>(data);
...
void on_registry_global(void* data,
  ...
  if (g_strcmp0(type, PW_TYPE_INTERFACE_Node) == 0) {
    ...
    if (const auto* key_media_class = spa_dict_lookup(props, PW_KEY_MEDIA_CLASS)) {
      ...
      if (std::ranges::find(PwPipelineManager::blocklist_node_name, node_name) != PwPipelineManager::blocklist_node_name.end()) {
        return;
      }
      ...
      if (!spa_dict_get_num(props, PW_KEY_OBJECT_SERIAL, serial)) {
        util::warning(
            "An error occurred while retrieving the object serial. The node cannot be handled by JamesDSP.");
        return;
      }
  ...
  if (g_strcmp0(type, PW_TYPE_INTERFACE_Link) == 0) {
    ...
    if (!spa_dict_get_num(props, PW_KEY_OBJECT_SERIAL, serial)) {
      util::warning(
          "An error occurred while retrieving the object serial. This link cannot be handled by JamesDSP.");
      return;
    }
  ...
  if (g_strcmp0(type, PW_TYPE_INTERFACE_Port) == 0) {
    ...
    if (!spa_dict_get_num(props, PW_KEY_OBJECT_SERIAL, serial)) {
      util::warning(
          "An error occurred while retrieving the object serial. This port cannot be handled by JamesDSP.");
      return;
    }
  ...
  if (g_strcmp0(type, PW_TYPE_INTERFACE_Module) == 0) {
    ...
    if (!spa_dict_get_num(props, PW_KEY_OBJECT_SERIAL, serial)) {
      util::warning(
          "An error occurred while retrieving the object serial. This module cannot be handled by JamesDSP.");
      return;
    }
  ...
  if (g_strcmp0(type, PW_TYPE_INTERFACE_Client) == 0) {
    ...
    if (!spa_dict_get_num(props, PW_KEY_OBJECT_SERIAL, serial)) {
      util::warning(
          "An error occurred while retrieving the object serial. This client cannot be handled by JamesDSP.");
      return;
    }
  ...
  if (g_strcmp0(type, PW_TYPE_INTERFACE_Device) == 0) {
    if (const auto* key_media_class = spa_dict_lookup(props, PW_KEY_MEDIA_CLASS)) {
      ...
      if (media_class == tags::pipewire::media_class::device) {
        ...
        if (!spa_dict_get_num(props, PW_KEY_OBJECT_SERIAL, serial)) {
          util::warning(
              "An error occurred while converting the object serial. This device cannot be handled by JamesDSP.");
          return;
        }
...
PwPipelineManager::PwPipelineManager() : header_version(pw_get_headers_version()), library_version(pw_get_library_version()) {
  ...
  thread_loop = pw_thread_loop_new("jdsp-pipewire-thread", nullptr);
