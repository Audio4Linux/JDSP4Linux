...
auto on_metadata_property(void* data, uint32_t id, const char* key, const char* type, const char* value) -> int {
    auto* pm = static_cast<PwPipelineManager*>(data);
    ...
    if (str_key == "default.audio.sink") {
        ...
        PwPipelineManager::json_object_find(str_value.c_str(), "name", v.data(), v.size() * sizeof(char));
    ...
    if (g_strcmp0(type, PW_TYPE_INTERFACE_Node) == 0) {
        if (const auto* key_media_role = spa_dict_lookup(props, PW_KEY_MEDIA_ROLE)) {
            ...
            if (g_strcmp0(key_media_role, "DSP") == 0) {
                ...
                if (g_strcmp0(key_media_category, "Filter") == 0) {
                    ...
                    if (g_strcmp0(key_node_description, "jamesdsp_filter") == 0) {
        ...
        if (const auto* key_media_class = spa_dict_lookup(props, PW_KEY_MEDIA_CLASS)) {
            ...
            if (media_class == "Audio/Sink" || media_class == "Audio/Source" || media_class == "Audio/Source/Virtual" ||
                ...
                if (media_class == "Audio/Source" && name != "jamesdsp_source") {
                    auto nd_info_copy = pd->nd_info;

                    Glib::signal_idle().connect_once([pm, nd_info_copy] { pm->source_added.emit(nd_info_copy); });
                } else if (media_class == "Audio/Sink" && name != "jamesdsp_sink") {
                    auto nd_info_copy = pd->nd_info;
...
PwPipelineManager::PwPipelineManager() {
    ...
    pw_properties_set(props_sink, PW_KEY_NODE_NAME, "jamesdsp_sink");
    pw_properties_set(props_sink, PW_KEY_NODE_DESCRIPTION, "JamesDSP Sink");
...
void PwPipelineManager::connect_stream_output(const uint& id, const std::string& media_class) const {
    if (media_class == "Stream/Output/Audio") {
        lock();
...
void PwPipelineManager::disconnect_stream_output(const uint& id, const std::string& media_class) const {
    if (media_class == "Stream/Output/Audio") {
        lock();
...
void PwPipelineManager::set_node_volume(pw_proxy* proxy, const int& n_vol_ch, const float& value) {
    std::array<float, SPA_AUDIO_MAX_CHANNELS> volumes{};
...
void PwPipelineManager::set_node_mute(pw_proxy* proxy, const bool& state) {
    std::array<char, 1024> buffer{};
...
void PwPipelineManager::lock() const {
    pw_thread_loop_lock(thread_loop);
}

void PwPipelineManager::unlock() const {
    pw_thread_loop_unlock(thread_loop);
}

void PwPipelineManager::sync_wait_unlock() const {
...
void PwPipelineManager::destroy_object(const int& id) const {
    lock();
...
void PwPipelineManager::destroy_links(const std::vector<pw_proxy*>& list) const {
    for (auto* proxy : list) {
        if (proxy != nullptr) {
            lock();
...
auto PwPipelineManager::json_object_find(const char* obj, const char* key, char* value, const size_t& len) -> int {
