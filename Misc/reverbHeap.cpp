...
static inline float randfloat() {
	...
	union { uint32_t i; float f; } u = { u.i = 0x3F800000 | R };
	return u.f - 1.0;
}
...
void sf_presetreverb(sf_reverb_state_st *rv, int rate, sf_reverb_preset preset, int allocateBuffer) {
...
#define CASE(prs, i)                                                                        \
		case prs: sf_advancereverb(rv, rate, ps[i].osf, ps[i].p1, ps[i].p2, ps[i].p3, ps[i].p4, \
			...
			ps[i].p13, ps[i].p14, ps[i].p15, ps[i].p16, allocateBuffer); return;
	switch (preset) {
...
void sf_advancereverb(sf_reverb_state_st *rv, int rate,
	...
	float basslpf, float damplpf, float outputlpf, float rt60, float delay, int allocateBuffer) {
	if (allocateBuffer) {
		rv->cdelayL.buf = (float*)calloc(SF_REVERB_DS, sizeof(float));
		rv->cdelayR.buf = (float*)calloc(SF_REVERB_DS, sizeof(float));
		rv->earlyref.delayPWL.buf = (float*)calloc(SF_REVERB_DS, sizeof(float));
		rv->earlyref.delayPWR.buf = (float*)calloc(SF_REVERB_DS, sizeof(float));
		rv->earlyref.delayRL.buf = (float*)calloc(SF_REVERB_DS, sizeof(float));
		rv->earlyref.delayLR.buf = (float*)calloc(SF_REVERB_DS, sizeof(float));
		rv->dampdL.buf = (float*)calloc(SF_REVERB_DS, sizeof(float));
		rv->dampdR.buf = (float*)calloc(SF_REVERB_DS, sizeof(float));
		rv->cbassd1L.buf = (float*)calloc(SF_REVERB_DS, sizeof(float));
		rv->cbassd1R.buf = (float*)calloc(SF_REVERB_DS, sizeof(float));
		rv->cbassd2L.buf = (float*)calloc(SF_REVERB_DS, sizeof(float));
		rv->cbassd2R.buf = (float*)calloc(SF_REVERB_DS, sizeof(float));
		rv->lastdelayL.buf = (float*)calloc(SF_REVERB_DS, sizeof(float));
		rv->lastdelayR.buf = (float*)calloc(SF_REVERB_DS, sizeof(float));
		rv->inpdelayL.buf = (float*)calloc(SF_REVERB_DS, sizeof(float));
		rv->inpdelayR.buf = (float*)calloc(SF_REVERB_DS, sizeof(float));
		rv->noise.buf = (float*)calloc(SF_REVERB_NS, sizeof(float));
		for (int i = 0; i < 4; i++) {
			rv->crossL[i].buf = (float*)calloc(SF_REVERB_APS, sizeof(float));
			rv->crossR[i].buf = (float*)calloc(SF_REVERB_APS, sizeof(float));
		}
		rv->cbassap1L.buf1 = (float*)calloc(SF_REVERB_AP2S1, sizeof(float));
		rv->cbassap1L.buf2 = (float*)calloc(SF_REVERB_AP2S2, sizeof(float));
		rv->cbassap1R.buf1 = (float*)calloc(SF_REVERB_AP2S1, sizeof(float));
		rv->cbassap1R.buf2 = (float*)calloc(SF_REVERB_AP2S2, sizeof(float));
		rv->cbassap2L.buf1 = (float*)calloc(SF_REVERB_AP3S1 + SF_REVERB_AP3M1, sizeof(float));
		rv->cbassap2L.buf2 = (float*)calloc(SF_REVERB_AP3S2, sizeof(float));
		rv->cbassap2L.buf3 = (float*)calloc(SF_REVERB_AP3S3, sizeof(float));
		rv->cbassap2R.buf1 = (float*)calloc(SF_REVERB_AP3S1 + SF_REVERB_AP3M1, sizeof(float));
		rv->cbassap2R.buf2 = (float*)calloc(SF_REVERB_AP3S2, sizeof(float));
		rv->cbassap2R.buf3 = (float*)calloc(SF_REVERB_AP3S3, sizeof(float));
		for (int i = 0; i < 10; i++) {
			rv->diffL[i].buf = (float*)calloc((SF_REVERB_APMS + SF_REVERB_APMM), sizeof(float));
			rv->diffR[i].buf = (float*)calloc((SF_REVERB_APMS + SF_REVERB_APMM), sizeof(float));
		}
		rv->dampap1L.buf = (float*)calloc((SF_REVERB_APMS + SF_REVERB_APMM), sizeof(float));
		rv->dampap1R.buf = (float*)calloc((SF_REVERB_APMS + SF_REVERB_APMM), sizeof(float));
		rv->dampap2L.buf = (float*)calloc((SF_REVERB_APMS + SF_REVERB_APMM), sizeof(float));
		rv->dampap2R.buf = (float*)calloc((SF_REVERB_APMS + SF_REVERB_APMM), sizeof(float));
		rv->combL.buf = (float*)calloc(SF_REVERB_CS, sizeof(float));
	}
...
void sf_reverb_free(sf_reverb_state_st *rv)
{
	free(rv->cdelayL.buf);
	free(rv->cdelayR.buf);
	free(rv->earlyref.delayPWL.buf);
	free(rv->earlyref.delayPWR.buf);
	free(rv->earlyref.delayRL.buf);
	free(rv->earlyref.delayLR.buf);
	free(rv->dampdL.buf);
	free(rv->dampdR.buf);
	free(rv->cbassd1L.buf);
	free(rv->cbassd1R.buf);
	free(rv->cbassd2L.buf);
	free(rv->cbassd2R.buf);
	free(rv->lastdelayL.buf);
	free(rv->lastdelayR.buf);
	free(rv->inpdelayL.buf);
	free(rv->inpdelayR.buf);
	free(rv->noise.buf);
	for (int i = 0; i < 4; i++) {
		free(rv->crossL[i].buf);
		free(rv->crossR[i].buf);
	}
	free(rv->cbassap1L.buf1);
	free(rv->cbassap1L.buf2);
	free(rv->cbassap1R.buf1);
	free(rv->cbassap1R.buf2);
	free(rv->cbassap2L.buf1);
	free(rv->cbassap2L.buf2);
	free(rv->cbassap2L.buf3);
	free(rv->cbassap2R.buf1);
	free(rv->cbassap2R.buf2);
	free(rv->cbassap2R.buf3);
	for (int i = 0; i < 10; i++) {
		free(rv->diffL[i].buf);
		free(rv->diffR[i].buf);
	}
	free(rv->dampap1L.buf);
	free(rv->dampap1R.buf);
	free(rv->dampap2L.buf);
	free(rv->dampap2R.buf);
	free(rv->combL.buf);
}