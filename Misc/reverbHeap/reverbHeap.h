...
//  Free the struct if no longer use...
//   sf_reverb_free(&rv);
// ---
//
...
#define SF_REVERB_DS        8000
typedef struct {
	int pos;                 // current write position
	int size;                // delay size
	float *buf; // delay buffer
} sf_rv_delay_st;
...
#define SF_REVERB_NS        (1<<14)
typedef struct {
	int pos;                 // current read position in the buffer
	float *buf; // buffer filled with noise
} sf_rv_noise_st;
...
#define SF_REVERB_AP2S1     9000
#define SF_REVERB_AP2S2     3400
typedef struct {
	//    line 1                 line 2
	int   pos1, pos2;
	int   size1, size2;
...
#define SF_REVERB_AP3S1     8100
#define SF_REVERB_AP3M1     683
#define SF_REVERB_AP3S2     4096
...
//
// note: this is about 1.5Mb, so you might not want to throw these around willy-nilly
typedef struct {
...
void sf_presetreverb(sf_reverb_state_st *state, int rate, sf_reverb_preset preset, int allocateBuffer);
...
void sf_advancereverb(sf_reverb_state_st *rv,
	...
	int allocateBuffer    // set to 1 when you initialize effect, no need reallocate any more if change parameters [0/1]
);
